...
其实有两种方法（在源代码中也可发现），一种是在用户代码中通过`app.run()`函数来启动，另一种方法是在命令行中用客户端来启动。
...
class BaseWSGIServer(HTTPServer, object):
    def serve_forever(self):
        ...
        try:
            HTTPServer.serve_forever(self)  # 这里
...
- 再看一下处理事件的函数。平平无奇，但是可以感觉到，这些都是一些预留插口等着上层逻辑去重写呢。
...
class Flask(_PackageBoundObject):
    def preprocess_request(self):
        ...
        如果执行过程中有任何一个 before_request_funcs 返回了 non-None 值，那么将被视为是view函数的返回值，其他的方法将不再执行。
        """
